...
class test_for_base_model(unittest.TestCase):
    ...
    def test_attr_none(self):
        """None attribute."""
        ob_test = BaseModel(None)
        self.assertTrue(hasattr(ob_test, "id"))
        self.assertTrue(hasattr(ob_test, "created_at"))
        self.assertTrue(hasattr(ob_test, "updated_at"))

    def test_kwargs_constructor_2(self):
        """ check id with data """
        dictonary = {'score': 100}

        ob_test = BaseModel(**dictonary)
        self.assertTrue(hasattr(ob_test, 'id'))
        self.assertTrue(hasattr(ob_test, 'created_at'))
        self.assertTrue(hasattr(ob_test, 'updated_at'))
        self.assertTrue(hasattr(ob_test, 'score'))
    ...
    def test_str(self):
        ...
        ob_test = BaseModel(**dictonary)
        out = "[{}] ({}) {}\n".format(type(ob_test).__name__, ob_test.id, ob_test.__dict__)

    def test_to_dict(self):
        """ check dict """
        ob_test = BaseModel(score=300)
        n_dict = ob_test.to_dict()

        self.assertEqual(n_dict['id'], ob_test.id)
        self.assertEqual(n_dict['score'], 300)
        self.assertEqual(n_dict['__class__'], 'BaseModel')
        self.assertEqual(n_dict['created_at'], ob_test.created_at.isoformat())
        self.assertEqual(n_dict['updated_at'], ob_test.updated_at.isoformat())
